...
class ProductEditor extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      categorySelector: false, // this to determine whether to open category selector
    }
  }
  ...
  closeCategorySelector = () => {
    // this function toggles category selector
    this.setState({
  ...
  openCategorySelector = () => {
    // this function for category input field focus
    this.setState({
  ...
  handleFileCheckbox = event => {
    // whether to open file selector
    this.setState({ openFileInputField: Boolean(event.target.checked) })
  }

  focusTitleEdit = () => {
    // edit button click and focus product title input field
    document.getElementById('product-title-input').focus()
  }

  render() {
    let { classes } = this.props
    let { userUploadImages, categorySelector, openFileInputField } = this.state
    let date = new Date()
    ...
    return (
      ...
      >
                ...
                {userUploadImages.length ? (
                  <div style={{ display: 'flex', flexWrap: 'wrap', }}>
                    {userUploadImages.map((image, index) => (
                      <ImageItem image={image} key={index} onClick={() => this.removeAnImage(image.name)} />
                    ))}
                  </div>
                ) : null}
              </Grid>
        ...
        <DialogContent style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', overflow: 'visible' }}>
          ...
          <ClickAwayListener onClickAway={this.closeCategorySelector}>
            <Paper elevation={0} className={classes.inputFields}>
              <IconButton aria-label="Menu" color='primary'>
                <Menu />
              </IconButton>
              <InputBase placeholder='Categories'
                onFocus={this.openCategorySelector}
              />
              {categorySelector ? (
                <CategorySelector />
              ) : null}
...
function ImageItem({ image, onClick }) {
  // state: openCloseBtn, default value: null
  // change state function: toggleCloseBtn
  const [openCloseBtn, toggleCloseBtn] = useState(false)
  const classes = makeStyles(editorStyle)()

  return (
    <span
      className={classes.imageSpan}
      onMouseEnter={() => toggleCloseBtn(!openCloseBtn)}
      onMouseLeave={() => toggleCloseBtn(!openCloseBtn)}
    >
      {openCloseBtn ? (
        <ButtonIcon
          iconName='unpin'
