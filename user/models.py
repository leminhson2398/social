...
class AppUser(models.Model):
    ...
    avatar = models.ImageField(upload_to='avatars/%Y/%m/%d', null=False, blank=True)
    date_of_birth = models.DateField(blank=True, null=True)
    quote_of_life = models.CharField(max_length=100, blank=True, null=True)
    gender = models.CharField(
        max_length=10, choices=Ref.GENDERS, null=False, blank=False)
    phone = models.CharField(max_length=15, null=True,
                             blank=True, validators=[validate_phone_number])
    role = models.CharField(
        max_length=10, choices=Ref.ROLES, default=Ref.ROLES[0][0])
...
class AvatarImage(models.Model):
    ...
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='images', null=False)
